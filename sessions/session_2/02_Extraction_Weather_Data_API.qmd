  ...
  if (httr2::resp_status(response) == 200) {
    ...
    purrr::map_dfr(
      ...
      ~ tibble::tibble(
        ...
        timestamp = .x$dt_txt,
        temp = .x$main$temp,
        weather = .x$weather[[1]]$description
      )
    )
  } else {
...
get_air_pollution_by_coords <- function(lat, lon) {
  ...
  if (httr2::resp_status(response) == 200) {
    ...
    tibble::tibble(
      aqi = data$list[[1]]$main$aqi,
      co = data$list[[1]]$components$co,
      pm2_5 = data$list[[1]]$components$pm2_5,
      pm10 = data$list[[1]]$components$pm10
    )
  } else {
    warning("Failed to retrieve air pollution data.")
    return(NULL)
  }
}
```
