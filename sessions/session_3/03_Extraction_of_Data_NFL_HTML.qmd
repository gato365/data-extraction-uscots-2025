...
```{r}
#| eval: false
# Load janitor for clean_names()
library(janitor)

# Clean and transform the data
history_df <- raw_history_df %>% 
  clean_names() %>% # Clean column names first
  filter(!is.na(as.numeric(____))) %>% # Remove repeating header rows
  mutate(across(c(____, ____, ____), as.numeric)) # Convert columns to numeric
...
history_df <- raw_history_df %>% 
  clean_names() %>% # Clean column names first
  filter(!is.na(as.numeric(year))) %>% # Remove repeating header rows
  mutate(across(c(year, w, l), as.numeric)) # Convert columns to numeric
